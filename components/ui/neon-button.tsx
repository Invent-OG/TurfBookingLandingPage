...
export const NeonButton = React.forwardRef<HTMLButtonElement, NeonButtonProps>(
  (
    { className, children, variant = "primary", glow = true, ...props },
    ref
  ) => {
    const variants = {
      primary:
    ...
    return (
      <button
        ref={ref}
        className={cn(
          "relative px-6 py-2.5 rounded-xl font-bold transition-all duration-300 flex items-center justify-center gap-2",
          variants[variant],
          glow ? glows[variant] : "",
          className
        )}
